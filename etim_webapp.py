...
def load_etim_data():
    ...
    if not righe_senza_sinonimi.empty:
        st.warning(
            f"🔍 Attenzione: {len(righe_senza_sinonimi)} classi non hanno sinonimi."
            " Considera di arricchirle per migliorare la precisione."
        )
...
# Funzione di classificazione semantica
...
st.markdown(
    "Inserisci una **descrizione**, carica un **PDF** o incolla un **link** "
    "per identificare la classe ETIM."
)
...
user_input = st.text_area(
    "📌 Oppure inserisci direttamente la descrizione del prodotto:",
    height=150
)

# Caricamento PDF
pdf_file = st.file_uploader(
    "📎 Carica una scheda tecnica in PDF (facoltativo):",
    type="pdf"
)
if pdf_file:
    try:
        with fitz.open(stream=pdf_file.read(), filetype="pdf") as doc:
            ...
            if len(user_input) < 50:
                st.warning(
                    "⚠️ Attenzione: il testo estratto dal PDF è molto breve "
...
url_input = st.text_input(
    "🔗 Oppure incolla un link a una scheda prodotto online:"
)
if url_input:
    try:
        ...
        text = ' '.join(
            p.get_text() for p in paragraphs if len(p.get_text()) > 30
        )
        user_input = text.strip()
        if len(user_input) < 50:
            st.warning(
                "⚠️ Attenzione: il testo estratto dal link è molto breve "
                "e potrebbe non essere sufficiente."
            )
        st.text_area(
            "🧾 Testo estratto dalla pagina:",
            value=user_input[:2000],
            height=150
        )
    except Exception as e:
...
if st.button("Classifica"):
    ...
    try:
        with st.spinner("Sto analizzando il significato della descrizione..."):
            ...
            st.markdown(
                f"✏️ Descrizione interpretata dall'AI: _{refined}_"
            )
            user_input = refined
    except Exception as e:
    ...
    if user_input:
        ...
        top_results = classify_semantic_top_k(
            user_input,
            df_etim,
            semantic_model,
            top_k=5
        ...
        if top_results.empty:
            st.error(
                "❌ Nessuna classe suggerita. Potresti arricchire "
                "i sinonimi nel file Excel."
            )
        else:
            ...
            for _, row in top_results.iterrows():
                ...
                st.markdown(
                    f"🔤 Descrizione (EN): {row['Description (EN)']}"
                )
                st.markdown(
                    f"📈 Confidenza AI: {row['Confidence']}%"
                )
                st.markdown("---")
    else:
        st.warning(
            "Inserisci una descrizione, carica un PDF o un link."
        )
